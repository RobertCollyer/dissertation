...
script, hid1, hid2, z_nodes, y_nodes, obj, z_noise, rlr, dglr, sslr, \
drop, dataset, decrease, n_train, n_test, epochs, ss_labels, seed, rep = argv
...
format(script, hid1, hid2, z_nodes, y_nodes, obj, z_noise, rlr, dglr, sslr, \
	drop, dataset, decrease, n_train, n_test, epochs, ss_labels, seed, rep)
print(name)
...
### decoder
yz_x = tf.concat([y_x,z_x_logits],1)
...
dy_fake, dy_fake_logits = xx.D_y(y_x, dy_weights) 

# z discriminator
z_real = xx.random_gaussian_laplace(z_noise,z_x_logits)
dz_real, dz_real_logits = xx.D_z(z_real, dz_weights)
dz_fake, dz_fake_logits = xx.D_z(z_x_logits, dz_weights) 
...
with tf.Session() as sess:
	...
	for epoch in range(epochs):
		...
		aex_test, acc, acc_t, yxlogt, zxlogt, dyr, dyf, dzr, dzf = \
		 sess.run([R_loss,SS_score,SS_score_t,y_x,z_x_logits,dy_real,dy_fake,dz_real,dz_fake], \
